...
def upload_file(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Limit the number of files in a batch
            max_limit = 600
            if len(files) > max_limit:
                messages.error(request, f"You can upload a maximum of {max_limit} files at a time.")
                return redirect('upload_file')  # Redirect back to the upload page
...
def batch_delete_files(request):
    if request.method == "POST":
        ...
        if file_ids:
            # Filter files by IDs and the current user
            files_to_delete = UploadedFile.objects.filter(file_id__in=file_ids, user=request.user)
            deleted_count = files_to_delete.count()
            
            # Delete files from both the file system and database
            for file in files_to_delete:
                file.delete()  # This removes it from the database and, ideally, the file system

            messages.success(request, f"Successfully deleted {deleted_count} files.")
        else:
            messages.warning(request, "No files selected for deletion.")
    return redirect("list_uploaded_files")

def batch_delete_confirm(request):
    if request.method == "POST":
        file_ids = request.POST.getlist('file_ids')
        files_to_delete = UploadedFile.objects.filter(file_id__in=file_ids, user=request.user)

        return render(request, 'core/confirm_batch_delete.html', {'files_to_delete': files_to_delete})
    return redirect('list_uploaded_files')
...
def list_uploaded_files(request):
    ...
    return render(request, 'core/file_list.html', {'page_obj': page_obj})
...
def create_album(request):
    if request.method == 'POST':
        ...
        if album_form.is_valid() and media_form.is_valid():
            ...
            return redirect('album_success')
        else:
            # If forms are invalid, add a general error message
            messages.error(request, 'Please fix the errors below.')
    else:
        album_form = AlbumForm()
        media_form = MediaFileSelectionForm(user=request.user)

    return render(request, 'core/create_album.html', {
        'album_form': album_form,
        'media_form': media_form,
    })

# Success View
def album_success(request):
    return render(request, 'core/album_success.html')
...
def album_list(request):
    ...
    return render(request, 'core/album_list.html', {'page_obj': page_obj})
...
#------------------------------------------------PLAYBACK MANAGEMENT------------------------------------------------#
@login_required
def playback_management(request):
    # Filter media files by the logged-in user and only include .mp3 and .mp4 files
    media_files = UploadedFile.objects.filter(
        user=request.user
    ).filter(
        file_name__endswith='.mp3'  # Filter for mp3 files
    ) | UploadedFile.objects.filter(
        user=request.user
    ).filter(
        file_name__endswith='.mp4'  # Filter for mp4 files
    )
    
    # Add is_audio and is_video properties dynamically
    for media in media_files:
        media.is_audio = media.file_name.endswith('.mp3')
        media.is_video = media.file_name.endswith('.mp4')

    return render(request, 'core/playback_management.html', {'media_files': media_files})
