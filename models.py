...
class Album(models.Model):
    ...
    def clean(self):
        # Custom model validation for album name
        if not self.album_name:
            raise ValidationError({'album_name': 'Requires album name'})

        # Custom validation for display pattern
        if not self.display_pattern:
            raise ValidationError({'display_pattern': 'Requires display pattern'})
...
class AlbumMediaFiles(models.Model):
    ...
    def clean(self):
        # Ensure that at least one media file is associated with the album
        if not self.album.albummediafiles_set.exists():
            raise ValidationError({'file': 'Requires at least one media file'})
